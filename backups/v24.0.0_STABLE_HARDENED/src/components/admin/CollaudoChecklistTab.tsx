...
export const CollaudoChecklistTab: React.FC<CollaudoChecklistTabProps> = ({ onUnsavedChange }) => {
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (!snap.empty) {
                    snap.docs.forEach(docSnap => {
                        ...
                        const key = docSnap.id as ChecklistKey;
                        if (key === 'rp' || key === 'sp') {
                            newLists[key] = d.items || [];
                        } else {
                            newLists[key] = d.items || [];
                            newTitles[key] = d.title || titles[key];
                        }
                    });
                    setLists(newLists);
                    setTitles(newTitles);
                } else if (settings.collaudoChecklists) {
                    // Fallback to settings if collection is empty
                    const s = settings.collaudoChecklists;
                    setLists({
                        rp: s.rp || [],
                        sp: s.sp || [],
                        c1: Array.isArray(s.c1) ? s.c1 : (s.c1?.items || []),
                        c2: Array.isArray(s.c2) ? s.c2 : (s.c2?.items || []),
                        c3: Array.isArray(s.c3) ? s.c3 : (s.c3?.items || []),
                        c4: Array.isArray(s.c4) ? s.c4 : (s.c4?.items || []),
                    });
                    setTitles({
                        c1: (s.c1 as any)?.title || 'Custom 1',
                        c2: (s.c2 as any)?.title || 'Custom 2',
                        c3: (s.c3 as any)?.title || 'Custom 3',
                        c4: (s.c4 as any)?.title || 'Custom 4',
                    });
                }
        ...
        loadChecklists();
    }, [settings.collaudoChecklists, isInitialized]);
    ...
    // Task 3: Checklist "Save All" Fix (Batch Update)
    const handleSave = async () => {
        ...
        try {
            ...
            // Loop through the checklist "rows" (categories)
            ...
            categories.forEach((key) => {
                ...
                // v10.1.1: Ensure arrays are wrapped in objects for Firestore documents
            ...
            // Commit the batch - Mandatory for stability
            await batch.commit();
            
            // Sync with settings for redundancy
            try {
                await updateSettings({ collaudoChecklists: data });
            } catch (e) {
                console.warn('[checklist] settings sync warning:', e);
            }
            
            setSaved(true);
            console.log('Batch update committed successfully for all checklists.');
        } catch (err: any) {
            console.error('Checklist Save All Failed:', err);
                        ...
                        {isCustom ? (
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                {key === 'rp' && (
                                    <button
                                        onClick={() => {
                                            if (window.confirm('Vuoi caricare le voci standard per RP? Le voci attuali verranno sovrascritte.')) {
                                                setLists(prev => ({ ...prev, rp: STANDARD_RP_CHECKLIST }));
                                                setSaved(false);
                                            }
                                        }}
                                        style={{
