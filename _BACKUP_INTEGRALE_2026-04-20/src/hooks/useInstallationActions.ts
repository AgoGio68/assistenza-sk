...
export const useInstallationActions = (
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Sincronizzazione con lo spreadsheet se presente una data di collaudo
            if (firestoreDoc.toTest && firestoreDoc.testDate) {
                const sheetType = (section === 's2' ? 'ordini_s2' : 'ordini') as 'ordini' | 'ordini_s2';
                syncInstallationStatus(finalDocId, true, sheetType).catch(err => 
                    console.error('[Sync] Errore sync collaudo:', err)
                );
            }
