    ...
    return (
        <div className="glass-panel" style={{ padding: '1.5rem', overflowX: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
                ...
                >
                    <div>
                        <h4 style={{ color: 'var(--success-color)', margin: 0, fontSize: '0.875rem' }}>
                            Ticket Chiusi
                        </h4>
                ...
                >
                    <div>
                        ...
                        <strong style={{ fontSize: '1.5rem', color: 'var(--warning-color)' }}>
            ...
            >
                <div>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <div
                            ...
                            title="Elimina tutti i ticket di un mese specifico"
                        >
                            <input
                                type="month"
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontSize: '0.8rem',
                                    opacity: deleteMonthStr ? 1 : 0.4,
                                    cursor: deleteMonthStr ? 'pointer' : 'not-allowed',
                                }}
                            >
                                Elimina Mese
                            </button>
                ...
                <div>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button
                            className={`btn ${ticketSortMode === 'chronological' ? 'btn-primary' : ''}`}
                            onClick={() => setTicketSortMode('chronological')}
                            style={{ padding: '0.4rem 1rem', fontSize: '0.8rem' }}
                            title="Mostra esattamente in ordine di data di apertura"
                        >
                            Cronologico
                        </button>
                        <button
                            className={`btn ${ticketSortMode === 'closed_bottom' ? 'btn-primary' : ''}`}
                            onClick={() => setTicketSortMode('closed_bottom')}
                            style={{ padding: '0.4rem 1rem', fontSize: '0.8rem' }}
                            title="Spingi i ticket già risolti in fondo"
                        >
                            Chiusi in Fondo
                        </button>
                    </div>
                            ...
                            filteredTickets.map((ticket, index) => (
                                ...
                                >
                                    <td style={{ padding: '1rem 0.75rem', fontSize: '0.875rem' }}>
                                        ...
                                        {ticket.closedAt && (
                                            <div style={{ marginTop: '0.25rem', color: 'var(--success-color)' }}>
                                    ...
                                    <td>
                                        <span
                                            className={`status-badge ${ticket.status === 'chiuso' ? 'chiuso' : ticket.status === 'aperto' ? 'aperto' : 'in-carico'}`}
                                        >
                                            {ticket.status}
                                        </span>
                                    </td>
                                    ...
                                    <td style={{ fontSize: '0.875rem', whiteSpace: 'nowrap' }}>
                                        {ticket.status === 'chiuso' && (
                                            <div style={{ color: 'var(--success-color)', fontWeight: 500 }}>
                                                <div>
                                                    Chiuso in:{' '}
                                                    {formatTimeDiff(ticket.createdAt, ticket.closedAt || Date.now())}
                                                </div>
                                                {(ticket.durationHours !== undefined ||
                                                    ticket.durationMinutes !== undefined) && (
                                                    <div
                                                        style={{
                                                            marginTop: '0.25rem',
                                                            fontSize: '0.75rem',
                                                    ...
                                                    >
                                                        Durata: {ticket.durationHours || 0}h{' '}
                                                        {ticket.durationMinutes || 0}m
                                                    </div>
                                                )}
                                            </div>
                                        )}
                                        {ticket.status !== 'chiuso' && (
                                            <span style={{ color: 'var(--warning-color)', fontWeight: 500 }}>
                                                Aperto da:
                                                <br />
                                                {formatTimeDiff(ticket.createdAt, Date.now())}
                                            </span>
                                        )}
                                    </td>
                                        ...
                                        >
                                            ...
                                            {ticket.status !== 'chiuso' && (
                                                ...
                                                >
                                                    ...
                                                    {users
                                                        .filter(
                                                            (u) => u.status === 'approved' && u.role !== 'superadmin',
                                                        )
                                                        .map((u) => (
