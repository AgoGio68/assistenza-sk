...
import { LayoutDashboard, PlusCircle, Settings, LogOut, Truck, Ticket, Calendar, Sun, Moon, Globe, Table, LogIn } from 'lucide-react';
...
export const Navigation: React.FC = () => {
    const { logout, isAdmin, currentUser, userProfile, userSections, isOffline } = useAuth();
    ...
    return (
        ...
        >
            {/* UX-02: Banner connessione lenta/offline */}
            {isOffline && (
                <div
                    style={{
                        position: 'fixed',
                ...
                >
                    ⚡ Connessione lenta o assente — alcuni dati potrebbero non essere aggiornati
                </div>
            )}
            {/* Left: Brand */}
            <NavLink
            ...
            >
                ...
                {!isAnonymous && userSections.includes('sk') && (
                ...
                {/* Ordini è accessibile a tutti (Admin o se si è sulla pagina via link pubblico) */}
                {(isAdmin || isAnonymous || window.location.pathname.includes('/sheet/ordini')) && (
                ...
                {settings.section2Enabled && (settings.section2InstallationsEnabled || isAdmin) && userSections.includes('s2') && (
                    <NavLink
                        to="/s2/installations"
                        className={navClass}
                ...
                {/* ─── Rapportini Quick Button (Solo registrati) ─── */}
                {!isAnonymous && (
                ...
                {isAnonymous ? (
                    ...
                    >
                        <LogIn size={22} /> <span className="hide-mobile">{t('login.submit')}</span>
                    </NavLink>
                ) : (
                    <button
                        onClick={async () => {
                            window.dispatchEvent(new CustomEvent('app-logout-cleanup'));
