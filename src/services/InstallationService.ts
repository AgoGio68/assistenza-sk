...
const parseCSV = (csvText: string): Installation[] => {
    ...
    return dataLines
        .map((parts, index): Installation | null => {
            ...
            const autoInvoiced =
                /FT/i.test(deliveryDateRaw) || (deliveryDateRaw.includes('_') && deliveryDateRaw.length > 5);
...
export const updateInstallationOnSheet = async (
    ...
    try {
        ...
        let targetSheetName = 'ORDINI'; // Il cliente garantisce che si chiama sempre ORDINI
        let targetGid = 0; // Fallback
        ...
        try {
            ...
            if (metaResponse.ok) {
                ...
                const targetSheet = meta.sheets?.find((s: any) => s.properties.title === targetSheetName);
                if (targetSheet) {
                    targetGid = targetSheet.properties.sheetId;
                } else {
                    console.warn(`[Sheets] Tab ${targetSheetName} non trovato, fallback su GID 0`);
                }
            }
        } catch (e) {
            console.warn('Could not fetch sheet metadata, stalling with ORDINI', e);
        }
        ...
        if (data.serialSK !== undefined) {
            ranges.push(`${targetSheetName}!G${rowIndex}`);
            values.push([data.serialSK]);
        }
        if (data.installDate !== undefined) {
            ranges.push(`${targetSheetName}!H${rowIndex}`);
            values.push([data.installDate]);
        }
        if (data.comments !== undefined) {
            ranges.push(`${targetSheetName}!I${rowIndex}`);
        ...
        if (ranges.length > 0) {
            ...
            if (!valResponse.ok) {
                // v3.3.0 Fix#3: errore 401 → messaggio esplicito per guida il refresh del token
                if (valResponse.status === 401) {
                    throw new Error(
                        'TOKEN_SCADUTO: Il collegamento a Google è scaduto (token OAuth valido ~1 ora). Ricollega Google dal tuo profilo o ricarica la pagina.',
                    );
                }
        ...
        let bgColor: any = whiteColor;
        if (data.status === 'tested') bgColor = greenColor;
        else if (data.status === 'toTest') bgColor = yellowColor;
...
export const appendInstallationToSheet = async (
    ...
    sheetName: string = 'ORDINI', // v3.3.0 Fix#4: nome tab configurabile
): Promise<string | null> => {
    try {
        const sheetIdMatch = spreadsheetUrl.match(/\/d\/([^/]+)/);
        if (!sheetIdMatch) throw new Error('URL foglio non valido.');
        ...
        try {
            ...
            if (metaResponse.ok) {
                const meta = await metaResponse.json();
                // v3.3.0 Fix#4: sheetName configurabile, con logging esplicito
                const targetSheet = meta.sheets?.find((s: any) => s.properties.title === (sheetName || 'ORDINI'));
                if (targetSheet) {
                    targetGid = targetSheet.properties.sheetId;
                } else {
                    const available = meta.sheets?.map((s: any) => s.properties.title).join(', ');
                    console.warn(
                        `[Sheets] Tab '${sheetName || 'ORDINI'}' non trovato per append. Tab disponibili: ${available}.`,
                    );
                }
            }
        } catch (e) {
        ...
        if (insertAtTop) {
            ...
            // 2. Scriviamo i valori in A2:Z2
            const updateReq = await fetch(
                `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${sheetName}!A2:Z2?valueInputOption=USER_ENTERED`,
                {
                    method: 'PUT',
                    headers: {
                        Authorization: `Bearer ${googleToken}`,
            ...
            if (!appendResponse.ok) {
                // v3.3.0 Fix#3: errore 401 → messaggio esplicito
                if (appendResponse.status === 401) {
                    throw new Error(
                        'TOKEN_SCADUTO: Il collegamento a Google è scaduto. Ricollega Google dal tuo profilo.',
                    );
                }
            ...
            if (newRowIndex) {
                const sheetStatus = data.tested ? 'tested' : (data.toTest ? 'toTest' : 'none');
                await updateInstallationOnSheet(spreadsheetUrl, googleToken, newRowIndex, { status: sheetStatus });
            }
