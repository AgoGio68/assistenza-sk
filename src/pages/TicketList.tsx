...
export const TicketList: React.FC<{ section?: 'sk' | 's2' }> = ({ section = 'sk' }) => {
    ...
    const [collaudoTicket, setCollaudoTicket] = useState<Ticket | null>(null);
    ...
    useEffect(() => {
        ...
        const conditions: any[] = [where('status', 'in', ['aperto', 'preso_in_carico'])];
        ...
        try {
            ...
            if (currentUser) {
                ...
                AuditLogService.logAction({
                    ...
                    action: 'UPDATE', 
                    resourceType: 'TICKET', 
                    resourceId: takeChargeTicket.id,
                    details: `${authorName} ha PRESO IN CARICO l'intervento per ${takeChargeTicket.companyName}.`
                });
            }
    ...
    const handleReassign = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (currentUser) {
                const authorName = userProfile?.displayName || currentUser.displayName || 'Utente';
                const targetUser = allUsers.find(u => u.uid === reassignTo);
                const targetName = targetUser?.displayName || 'un collega';
                AuditLogService.logAction({
                    userId: currentUser.uid, 
                    userEmail: currentUser.email || '', 
                    userName: authorName, 
                    ...
                    action: 'ASSIGN', 
                    resourceType: 'TICKET', 
                    resourceId: reassignTarget.ticketId,
                    details: `${authorName} ha RIASSEGNATO l'intervento di ${reassignTarget.oldAssigneeName} a ${targetName}. Motivo: ${reassignReason || 'Nessuno specifico'}`
                });
            }
        ...
        try {
            ...
            if (currentUser) {
                ...
                AuditLogService.logAction({
                    ...
                    resourceType: 'TICKET', 
                    resourceId: ticketId,
                    details: `${authorName} ha CHIUSO l'intervento per ${ticket?.companyName || 'un cliente'}. Durata: ${hours}h ${minutes}m.`
                });
            }
        } catch (err) {
            console.error(err);
        ...
        try {
            ...
            if (currentUser) {
                ...
                AuditLogService.logAction({
                    ...
                    resourceType: 'TICKET', 
                    resourceId: ticketId,
                    details: `${authorName} ha ELIMINATO DEFINITIVAMENTE il ticket per ${ticketToDelete?.companyName || 'un cliente'}.`
                });
            }
        ...
        <div style={{ paddingBottom: '2rem' }}>
                ...
                <div>
                    <h2
                        style={{
                            ...
                            color:
                                section === 's2'
                                    ? settings.section2Color || 'var(--accent-teal)'
                                    : 'var(--text-primary)',
                            marginBottom: '0.1rem',
                        }}
                    >
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    animation: ticket.highlighted
                                        ? 'glowPulse 1.4s ease-in-out infinite'
                                        : 'slideInUp 0.3s ease forwards',
                                    position: 'relative',
                                    ...
                                    >
                                        <div style={{ marginBottom: '0.2rem', color: 'var(--text-muted)' }}>
                                            <span style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>
                                                {getCreatorName(ticket, allUsers)}
                                            </span>
                                        </div>
                                        <div
                                            style={{
                                                fontSize: '0.8rem',
                                                fontWeight: 500,
                                                color: isTakenByMe ? 'var(--accent-teal)' : 'var(--text-secondary)',
                                            }}
                                        >
                                        ...
                                        {ticket.scheduledDate && (
                                            ...
                                            >
                                                <Calendar size={11} />{' '}
                                                {new Date(ticket.scheduledDate).toLocaleString([], {
                                                    day: '2-digit',
                                                    month: '2-digit',
                                                    hour: '2-digit',
                                                    minute: '2-digit',
                                ...
                                >
                                    ...
                                    <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                handleToggleHighlight(ticket.id!, !!ticket.highlighted);
                                            }}
                                            style={{
                                                background: 'none',
                                                border: 'none',
                                        ...
                                        {isTakenByOthers && (
                                            <span title={assigneeName}>
                                                <UserPlus size={14} color="var(--warning-color)" />
                                            </span>
                                        )}
                    ...
                    return (
                        <div
                            ...
                            style={{
                                ...
                                opacity: isTakenByOthers ? 0.55 : 1,
                                boxShadow: ticket.highlighted
                                    ? '0 0 20px rgba(245,158,11,0.35), var(--shadow-sm)'
                                    : 'var(--shadow-sm)',
                                animation: ticket.highlighted
                                    ? 'glowPulse 1.4s ease-in-out infinite'
                                    : 'slideInUp 0.3s ease forwards',
                        ...
                        >
                            ...
                            >
                                <div
                                    style={{
                                        fontWeight: 700,
                                        fontSize: '1.05rem',
                                        color: isUrgent ? 'var(--danger-color)' : 'var(--text-primary)',
                                    }}
                                >
                                    {ticket.companyName}
                                </div>
                            ...
                            <div style={{ marginBottom: '0.8rem', color: 'var(--text-secondary)' }}>
                                <div style={{ fontSize: '0.85rem', marginBottom: '0.25rem' }}>
                                    <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Ref:</span>{' '}
                                    <strong style={{ color: 'var(--text-primary)' }}>{ticket.contactName}</strong>
                                    {ticket.phone && ticket.phone.trim() !== '' && ticket.phone.trim() !== '()' ? (
                                        <span style={{ color: 'var(--text-muted)' }}> · {ticket.phone.trim()}</span>
                                    ) : (
                                        ''
                                    )}
                                </div>
                            ...
                            {isTakenByOthers && isAdmin && (
                                ...
                                >
                                    <UserPlus size={14} /> Preso in carico{' '}
                                    {showOtherAssignees ? `da: ${assigneeName}` : 'da un collega'}
                                </div>
                            )}
                            ...
                            {isTakenByOthers && !isAdmin ? (
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                                        <button
                                            ...
                                            title={
                                                ticket.highlighted
                                                    ...
                                                    : 'Evidenzia ticket (Lampeggio)'
                                            }
                                            style={{
                                                padding: '0.65rem 0.75rem',
                                                background: ticket.highlighted
                                        ...
                                        >
                                            <Zap size={17} fill={ticket.isCollaudo ? '#facc15' : 'none'} />{' '}
                                            {ticket.isCollaudo ? 'Collaudo' : 'Segna Collaudo'}
                                        </button>
                                        {ticket.isCollaudo && (
                                            <button
                                                onClick={() => setCollaudoTicket(ticket)}
                                                className="btn"
                                    ...
                                    {ticket.status === 'preso_in_carico' &&
                                        isAdmin &&
                                        !isTakenByMe &&
                                        settings.adminCanReassignOthers && (
                                            <button
                                                onClick={() =>
                                                    setReassignTarget({
                                                        ticketId: ticket.id!,
                                                        oldAssigneeName: assigneeName,
                                                    })
                                                }
                                                className="btn"
                                                style={{
            ...
            {selectedTicket && (
                ...
                >
                    ...
                    >
                        ...
                        >
                            <div
                                style={{
                                    ...
                                    height: 36,
                                    borderRadius: 4,
                                    background: selectedTicket.isCollaudo
                                        ? '#facc15'
                                        : selectedTicket.urgency === 'urgente'
                                          ? 'var(--danger-color)'
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <h3
                                        style={{
                                            ...
                                            fontSize: '1.15rem',
                                            fontWeight: 700,
                                            color: selectedTicket.isCollaudo
                                                ? '#facc15'
                                                : selectedTicket.urgency === 'urgente'
                                                  ? 'var(--danger-color)'
                                                  : 'var(--text-primary)',
                        ...
                        >
                            {[
                                ['Creato da', getCreatorName(selectedTicket, allUsers)],
                                [
                                    'Referente',
                                    `${selectedTicket.contactName}${selectedTicket.phone && selectedTicket.phone.trim() !== '' && selectedTicket.phone.trim() !== '()' ? ' · ' + selectedTicket.phone.trim() : ''}`,
                                ],
                                ['In carico a', getAssigneeName(selectedTicket, allUsers)],
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1rem' }}>
                            {selectedTicket.status === 'aperto' && (
                                <button
                                    onClick={() => {
                                        handleTakeCharge(selectedTicket);
                                        setSelectedTicket(null);
                                    }}
                                    className="btn btn-primary"
                                    style={{ flex: 1 }}
                            ...
                            {(isAdmin || selectedTicket.assignedTo === currentUser?.uid) && (
                                <button
                                    onClick={() => handleUpdateNotes(selectedTicket.id!, selectedTicket.notes)}
                                    className="btn"
                            ...
                            {selectedTicket.isCollaudo && (
                                <button
                                    onClick={() => {
                                        setCollaudoTicket(selectedTicket);
                                        setSelectedTicket(null);
                                    }}
                                    className="btn"
                                    style={{
                                    ...
                                    <button
                                        onClick={() =>
                                            setReassignTarget({
                                                ...
                                                oldAssigneeName:
                                                    userNames[selectedTicket.assignedTo!] || selectedTicket.assignedTo!,
                                            })
                                        }
                                        className="btn"
                                        style={{
            ...
            {reassignTarget && (
                ...
                >
                    ...
                    >
                        <h3 style={{ marginBottom: '1.25rem', color: 'var(--text-primary)', fontSize: '1.1rem' }}>
                            Riassegna Ticket
                        </h3>
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ marginBottom: '1.25rem' }}>
                            ...
                            >
                                Pianifica data e ora (opzionale)
                            </label>
                            <input
                                type="datetime-local"
            ...
            {collaudoTicket && (
                <CollaudoChecklistModal
                    installationId={`ticket-${collaudoTicket.id}`}
                    machineName={collaudoTicket.description?.split(' ')[0] || 'generic'}
                    clientName={collaudoTicket.companyName}
                    scheduledDate={collaudoTicket.scheduledDate || collaudoTicket.testDate}
                    onClose={() => setCollaudoTicket(null)}
                />
            )}
            ...
            {usageModal.isOpen && (
                <InventoryUsageModal
                    ...
                    referenceId={usageModal.ticketId}
                    referenceType="ticket"
                    referenceName={usageModal.companyName}
                />
            )}
        </div>
    );
