...
export const Profile: React.FC = () => {
    const { currentUser, userProfile, updateDisplayName, updatePhone } = useAuth();
    ...
    const handleSavePhone = async () => {
        try {
            // Pulizia base: solo numeri
            const clean = newPhone.replace(/\D/g, '');
            await updatePhone(clean);
            setIsEditingPhone(false);
        } catch (err) {
            alert("Errore durante l'aggiornamento del telefono.");
        }
    };
                ...
                >
                    <div style={{ flex: 1, minWidth: '250px' }}>
                        ...
                        <div style={{ marginBottom: '1.5rem' }}>
                            ...
                            >
                                Telefono WhatsApp (es. 393331234567)
                            </div>
                            {!isEditingPhone ? (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                    <strong
                                        style={{
                                            fontSize: '1.2rem',
                                            color: userProfile?.phone ? 'var(--text-primary)' : 'var(--text-muted)',
                                        }}
                                    >
                                        {userProfile?.phone || 'Non inserito'}
                                    </strong>
                                    <button
                            ...
                            <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                * Necessario per ricevere notifiche su nuovi ticket.
                            </p>
                        </div>
