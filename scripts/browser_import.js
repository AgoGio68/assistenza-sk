...
async function importViaBrowser() {
    ...
    dataLines.forEach((line, r) => {
        // Parsing CSV semplice (split per virgola, gestendo eventuali virgolette se necessario, ma qui l'utente ha mandato dati piatti)
        // Per sicurezza usiamo un regex che rispetta le virgole
        const segments = line.split(',');
        segments.forEach((val, c) => {
            if (val && val.trim() !== '') {
                const cell = colLetter(c) + (r + 2); // Riga 1 è header, quindi r+2
                cellMap[cell] = val.trim();
            }
        });
    });

    console.log(`Piano di importazione generato: ${Object.keys(cellMap).length} celle.`);

    const ref = db.collection('fogli_condivisi').doc('ordini');
    
    // Azzeriamo tutto prima o sovrascriviamo? Sovrascriviamo solo i campi.
    // L'utente vuole anche pulire i commenti.
    await ref.update({
        data: cellMap,
        comments: {}
    });
...
importViaBrowser().catch(console.error);
